package com.bluedon.entity;

import java.util.Date;

/**
 * Created by dengxiwen on 2016/12/27.
 */
public class TSiemGeneralLog {
    private String recordid;
    private String eventdefid;
    private Integer eventlevel;
    private Date storagetime;
    private String orilevel;
    private String proto;
    private String reportnetype;
    private String reportvendor;
    private String reportneid;
    private String reportip;
    private String reportapp;
    private Date firstrecvtime;
    private Date lastrecvtime;
    private String sourceip;
    private String sourceport;
    private String sourceneid;
    private String destip;
    private String destport;
    private String destneid;
    private String eventaction;
    private String actionresult;
    private Integer repeatcount;
    private String stringname1;
    private String stringvalue1;
    private String stringname2;
    private String stringvalue2;
    private String stringname3;
    private String stringvalue3;
    private String stringname4;
    private String stringvalue4;
    private String stringname5;
    private String stringvalue5;
    private String eventname;
    private String eventsubtype;
    private String logeventtype;
    private String dublecount;

    public String getRecordid() {
        return this.recordid;
    }

    public void setRecordid(String recordid) {
        this.recordid = recordid;
    }

    public String getEventdefid() {
        return this.eventdefid;
    }

    public void setEventdefid(String eventdefid) {
        this.eventdefid = eventdefid;
    }

    public Integer getEventlevel() {
        return this.eventlevel;
    }

    public void setEventlevel(Integer eventlevel) {
        this.eventlevel = eventlevel;
    }

    public Date getStoragetime() {
        return this.storagetime;
    }

    public void setStoragetime(Date storagetime) {
        this.storagetime = storagetime;
    }

    public String getOrilevel() {
        return this.orilevel;
    }

    public void setOrilevel(String orilevel) {
        this.orilevel = orilevel;
    }

    public String getProto() {
        return this.proto;
    }

    public void setProto(String proto) {
        this.proto = proto;
    }

    public String getReportnetype() {
        return this.reportnetype;
    }

    public void setReportnetype(String reportnetype) {
        this.reportnetype = reportnetype;
    }

    public String getReportvendor() {
        return this.reportvendor;
    }

    public void setReportvendor(String reportvendor) {
        this.reportvendor = reportvendor;
    }

    public String getReportneid() {
        return this.reportneid;
    }

    public void setReportneid(String reportneid) {
        this.reportneid = reportneid;
    }

    public String getReportip() {
        return this.reportip;
    }

    public void setReportip(String reportip) {
        this.reportip = reportip;
    }

    public String getReportapp() {
        return this.reportapp;
    }

    public void setReportapp(String reportapp) {
        this.reportapp = reportapp;
    }

    public Date getFirstrecvtime() {
        return this.firstrecvtime;
    }

    public void setFirstrecvtime(Date firstrecvtime) {
        this.firstrecvtime = firstrecvtime;
    }

    public Date getLastrecvtime() {
        return this.lastrecvtime;
    }

    public void setLastrecvtime(Date lastrecvtime) {
        this.lastrecvtime = lastrecvtime;
    }

    public String getSourceip() {
        return this.sourceip;
    }

    public void setSourceip(String sourceip) {
        this.sourceip = sourceip;
    }

    public String getSourceport() {
        return this.sourceport;
    }

    public void setSourceport(String sourceport) {
        this.sourceport = sourceport;
    }

    public String getSourceneid() {
        return this.sourceneid;
    }

    public void setSourceneid(String sourceneid) {
        this.sourceneid = sourceneid;
    }

    public String getDestip() {
        return this.destip;
    }

    public void setDestip(String destip) {
        this.destip = destip;
    }

    public String getDestport() {
        return this.destport;
    }

    public void setDestport(String destport) {
        this.destport = destport;
    }

    public String getDestneid() {
        return this.destneid;
    }

    public void setDestneid(String destneid) {
        this.destneid = destneid;
    }

    public String getEventaction() {
        return this.eventaction;
    }

    public void setEventaction(String eventaction) {
        this.eventaction = eventaction;
    }

    public String getActionresult() {
        return this.actionresult;
    }

    public void setActionresult(String actionresult) {
        this.actionresult = actionresult;
    }

    public Integer getRepeatcount() {
        return this.repeatcount;
    }

    public void setRepeatcount(Integer repeatcount) {
        this.repeatcount = repeatcount;
    }

    public String getStringname1() {
        return this.stringname1;
    }

    public void setStringname1(String stringname1) {
        this.stringname1 = stringname1;
    }

    public String getStringvalue1() {
        return this.stringvalue1;
    }

    public void setStringvalue1(String stringvalue1) {
        this.stringvalue1 = stringvalue1;
    }

    public String getStringname2() {
        return this.stringname2;
    }

    public void setStringname2(String stringname2) {
        this.stringname2 = stringname2;
    }

    public String getStringvalue2() {
        return this.stringvalue2;
    }

    public void setStringvalue2(String stringvalue2) {
        this.stringvalue2 = stringvalue2;
    }

    public String getStringname3() {
        return this.stringname3;
    }

    public void setStringname3(String stringname3) {
        this.stringname3 = stringname3;
    }

    public String getStringvalue3() {
        return this.stringvalue3;
    }

    public void setStringvalue3(String stringvalue3) {
        this.stringvalue3 = stringvalue3;
    }

    public String getStringname4() {
        return this.stringname4;
    }

    public void setStringname4(String stringname4) {
        this.stringname4 = stringname4;
    }

    public String getStringvalue4() {
        return this.stringvalue4;
    }

    public void setStringvalue4(String stringvalue4) {
        this.stringvalue4 = stringvalue4;
    }

    public String getStringname5() {
        return this.stringname5;
    }

    public void setStringname5(String stringname5) {
        this.stringname5 = stringname5;
    }

    public String getStringvalue5() {
        return this.stringvalue5;
    }

    public void setStringvalue5(String stringvalue5) {
        this.stringvalue5 = stringvalue5;
    }

    public String getEventname() {
        return this.eventname;
    }

    public void setEventname(String eventname) {
        this.eventname = eventname;
    }

    public String getEventsubtype() {
        return this.eventsubtype;
    }

    public void setEventsubtype(String eventsubtype) {
        this.eventsubtype = eventsubtype;
    }

    public String getLogeventtype() {
        return this.logeventtype;
    }

    public void setLogeventtype(String logeventtype) {
        this.logeventtype = logeventtype;
    }

    public String getDublecount() {
        return this.dublecount;
    }

    public void setDublecount(String dublecount) {
        this.dublecount = dublecount;
    }
}
