...
object AlarmGLAnalyzeServer {
  ...
  def main(args: Array[String]) {
    ...
    try{
      ...
      //监听批处理
      ssc_alarm_gl.addStreamingListener(kafkaStreamListener)

      //告警处理-联合事件
      val dataProcess = new DataProcess
      dataProcess.alarmProcessForGL(spark,ssc_alarm_gl,properties,bcVarUtil)
      ssc_alarm_gl.start()
