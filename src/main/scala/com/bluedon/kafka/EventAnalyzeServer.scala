...
object EventAnalyzeServer {
  ...
  def main(args: Array[String]) {
    ...
    try{
      ...
      //监听批处理
      ssc_event.addStreamingListener(kafkaStreamListener)

      //事件处理
      val dataProcess = new DataProcess
      dataProcess.eventProcess(spark,ssc_event,properties,bcVarUtil)
      ssc_event.start()
